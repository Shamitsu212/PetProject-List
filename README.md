# Portfolio

Welcome!

This repository contains my frontend projects, ranging from small learning exercises to production-like applications.
My main focus is building modern web applications with **React** and **TypeScript**.

---

# ⭐ Featured Projects

### 🌐 Portfolio Website

Personal portfolio website showcasing my projects, skills, and experience. Built with React and designed to provide a clean, responsive, and modern user experience.

**Repository:** https://github.com/Shamitsu212/portfolio-website  
**Live Demo:** https://shamitsu212.github.io/portfolio-website/

---

### 📋 Todo List
React application for managing daily tasks.

**Repository:** https://github.com/Shamitsu212/React_TodoList  
**Live Demo:** https://shamitsu212.github.io/React_TodoList/

---

### 📝 Note App
Simple note-taking application built with React.

**Repository:** https://github.com/Shamitsu212/React_NoteApp  
**Live Demo:** https://shamitsu212.github.io/React_NoteApp/

---

### 🎴 Anki Cards
Flashcard application for learning with spaced repetition.

**Repository:** https://github.com/Shamitsu212/AnkiCards_React  
**Live Demo:** https://shamitsu212.github.io/AnkiCards_React/

---

# Commercial & Test Work

Projects completed for freelance clients or as technical assessments.

<div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
  <a href="https://github.com/Shamitsu212/TEST_NDM" target="_blank">
      Test work "NDM systems"
  </a>
 </div>

<div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
  <a href="https://github.com/Shamitsu212/resume_creator" target="_blank">
      Test work "OOO World Technology"
  </a>
</div>

<div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
  <a href="https://github.com/Shamitsu212/freelance_website" target="_blank">
    Freelance MedX Landing Page
  </a>
</div>

---

# Layout Projects

Responsive interfaces built from Figma designs using HTML, React and Vue.

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/figma-to-html" target="_blank">
      HTML Landing Page
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Adaptive-layout" target="_blank">
      Adaptive HTML Landing Page #1
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Adaptive_layout2" target="_blank">
      Adaptive HTML Landing Page #2
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Figma_to_React" target="_blank">
      React Landing Page
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Figma_to_VUE" target="_blank">
      VUE Landing Page
    </a>
  </div>


---

# CI/CD

Projects demonstrating CI/CD pipelines, deployment automation and development workflows.

<div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
  <a href="https://github.com/Shamitsu212/CI_CD_exercise" target="_blank">
    GitHub Actions CI/CD Demo
  </a>
</div>

---
# Learning Projects

Small projects created while learning React and Vue concepts.

### <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="22" style="vertical-align: middle;" /> React

<div style="display: flex; flex-wrap: wrap; gap: 20px;">
  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_counter" target="_blank">
      Counter App
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_ModalWindow/tree/main" target="_blank">
       Modal Component
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_Quiz" target="_blank">
       Quiz App
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_UserList" target="_blank">
      User Directory
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/currency_converter" target="_blank">
      Currency Converter
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/collection_react" target="_blank">
      Collection Explorer
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Book-Recommendation-System" target="_blank">
      Book Finder by Google Book API
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/ReactAndRedux_shop" target="_blank">
      StoreFront
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/REST_API_exercise" target="_blank">
      REST API Playground
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_RouterDOM_exercise" target="_blank">
      Routing Demo
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/React_Context_exercise" target="_blank">
      Context API Demo
    </a>
  </div>
  
</div>

---

### <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg" width="22" style="vertical-align: middle;" /> Vue

<div style="display: flex; flex-wrap: wrap; gap: 20px;">
  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Vue_counter" target="_blank">
      Counter App
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Vue_ModalWindow" target="_blank">
      Modal Component
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Vue_Quiz" target="_blank">
       Quiz App
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Vue_UserList" target="_blank">
       User Directory
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/Vue_currency_converter" target="_blank">
      Currency Converter
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/collection_vue" target="_blank">
      Collection Explorer
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/VUE_Post_Application" target="_blank">
      API Client
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/VueAndPinia_shop" target="_blank">
      StoreFront
    </a>
  </div>
</div>

---

### <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nestjs/nestjs-original.svg" width="22" style="vertical-align: middle;" /> Backend Experiments

Small backend projects created to better understand server-side development, authentication, databases and API design from a frontend developer's perspective.

<div style="display: flex; flex-wrap: wrap; gap: 20px;">
  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/user_CRUD_API" target="_blank">
      Simple CRUD
    </a>
  </div>
</div>

<div style="display: flex; flex-wrap: wrap; gap: 20px;">
  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/nestjs-basic-server" target="_blank">
      JWT Authentication API
    </a>
  </div>
</div>

---

### <img src="https://www.codewars.com/packs/assets/logo.f607a0fb.svg" width="22" style="vertical-align: middle;" /> Coding Practice

Solutions to algorithmic challenges and coding exercises completed on Codewars to improve problem-solving skills and deepen knowledge of JavaScript.

<div style="display: flex; flex-wrap: wrap; gap: 20px;">
  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://github.com/Shamitsu212/codewars-solutions" target="_blank">
      Codewars Solutions
    </a>
  </div>

  <div style="border: 1px solid #ddd; border-radius: 10px; padding: 15px; width: 260px;">
    <a href="https://www.codewars.com/users/Shamitsu212" target="_blank">
      Codewars Profile
    </a>
  </div>
</div>
